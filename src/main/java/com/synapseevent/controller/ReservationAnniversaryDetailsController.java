...
public class ReservationAnniversaryDetailsController {
    ...
    private AnniversaryEvent currentEvent;
    ...
    private void loadEventDetails(Long eventId) {
        try {
            currentEvent = anniversaryEventService.findbyId(eventId);
        ...
        if (currentEvent == null) {
            showError("Anniversaire non trouvé");
            return;
        }
    ...
    private void makeReservation(int seats) {
        ...
        // Make reservation
        try {
            Booking booking = new Booking(userId, "ANNIVERSARY", currentEvent.getId(), LocalDate.now(), "confirmed");
