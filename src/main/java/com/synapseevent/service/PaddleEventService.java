...
public class PaddleEventService implements IService<PaddleEvent> {
    ...
    public boolean ajouter(PaddleEvent event) throws SQLException {
        String sql = "INSERT INTO PaddleEvent (name, date, description, status) VALUES (?, ?, ?, ?)";
    ...
    public List<PaddleEvent> readAll() throws SQLException {
        ...
        try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                PaddleEvent event = new PaddleEvent(rs.getLong("id"), rs.getString("name"), rs.getDate("date").toLocalDate(), rs.getString("description"), rs.getString("status"));
                events.add(event);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public PaddleEvent findbyId(Long id) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                return new PaddleEvent(rs.getLong("id"), rs.getString("name"), rs.getDate("date").toLocalDate(), rs.getString("description"), rs.getString("status"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
            throw e;
    ...
    public boolean modifier(PaddleEvent event) throws SQLException {
        String sql = "UPDATE PaddleEvent SET name = ?, date = ?, description = ?, status = ? WHERE id = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, event.getName());
            stmt.setDate(2, Date.valueOf(event.getDate()));
            stmt.setString(3, event.getDescription());
