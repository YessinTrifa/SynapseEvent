...
public class FormationEventService implements IService<FormationEvent> {
    ...
    public boolean ajouter(FormationEvent event) throws SQLException {
        String sql = "INSERT INTO FormationEvent (name, date, description) VALUES (?, ?, ?)";
    ...
    public List<FormationEvent> readAll() throws SQLException {
        ...
        try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                FormationEvent event = new FormationEvent(rs.getLong("id"), rs.getString("name"), rs.getDate("date").toLocalDate(), rs.getString("description"));
                events.add(event);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public FormationEvent findbyId(Long id) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                return new FormationEvent(rs.getLong("id"), rs.getString("name"), rs.getDate("date").toLocalDate(), rs.getString("description"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
            throw e;
    ...
    public boolean modifier(FormationEvent event) throws SQLException {
        String sql = "UPDATE FormationEvent SET name = ?, date = ?, description = ? WHERE id = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, event.getName());
            stmt.setDate(2, Date.valueOf(event.getDate()));
            stmt.setString(3, event.getDescription());
